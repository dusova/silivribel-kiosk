...
async function openInBrowserView(url, id) {
    ...
    activeBrowserView = new BrowserView({
        webPreferences: {
            ...
            sandbox: true,
            backgroundThrottling: false, // Arka planda olsa da haritayı dondurma
            enableWebSQL: false,
            spellcheck: false,
        },
    });
...
function createMainWindow() {
    ...
    if (!isDev) {
        // Start Menu ve Edge Panel'i ezmek için en üst seviye odaklama
        mainWindow.setAlwaysOnTop(true, 'screen-saver');
    }

    // Güvenlik: izinleri yalnızca belirli origin'lere ve belirli türlere kısıtla.
    // Tüm origin'lere toplu 'media' izni vermek, herhangi bir izin verilen
    // sitenin kullanıcı onayı olmadan mikrofon/kamera açmasına yol açar.
    ...
    mainWindow.webContents.session.setPermissionRequestHandler(
        (webContents, permission, callback) => {
            ...
            // media, notifications ve diğer tüm izinleri reddet
            callback(false);
        }
    );
...
app.whenReady().then(() => {
    ...
    // Optimizasyon: AdBlocker'\u0131 session seviyesinde bir kez kaydet (F15)
    ...
    // Sağ tık (Context Menu) global olarak tüm sayfalarda engelle
