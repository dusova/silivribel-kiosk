...
function showScreen(name) {
    ...
    if (name === 'splash') {
        stopIdle();
        // Optimizasyon: splash ekranında saat güncellemesini durdur (F9)
        if (clockInterval) { clearInterval(clockInterval); clockInterval = null; }
        document.getElementById('timeout-overlay').style.display = 'none';
    } else {
        // Optimizasyon: main ekrana geçince saati başlat (F9)
        if (!clockInterval) {
            updateClock();
...
function applySettings() {
    ...
    if (!raw) return;
    // Güvenlik: CSS url() enjeksiyonunu önlemek için URL'yi doğrula.
    // Yalnızca https: protokolüne izin ver; data:, javascript: vb. bloklansın.
    try {
        const u = new URL(raw);
        if (u.protocol !== 'https:') return;
        // Gömülü çift tırnak karakterini yüzde-kodla — CSS url() kaçışı
...
async function fetchServices() {
    ...
    try {
        ...
        // Optimizasyon: DocumentFragment ile tek DOM mutation (F18)
...
async function init() {
    setupIPC();
    updateClock();
    // Optimizasyon: Saat interval'i showScreen('main') içinde başlatılır (F9)

    // Güvenlik: inline onclick kaldırıldı — event listener'lar buradan eklenir
